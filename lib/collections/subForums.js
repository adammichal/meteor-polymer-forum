...
Meteor.methods({
	addSubForum: function (forumId, name, icon, description) {
		if (!Roles.userIsInRole(this.userId, forumConfig.forum.addSubForumRoles)) {
	...
	editSubForum: function (id,update) {
		if (!Roles.userIsInRole(this.userId, forumConfig.forum.editSubForumRoles)) {
			throw new Meteor.Error(403, "You're not autorized!");
		} else {
			subForums.update({
				_id: id
	...
	deleteSubForum: function (id) {
		if (!Roles.userIsInRole(this.userId, forumConfig.forum.deleteSubForumRoles)) {
			throw new Meteor.Error(403, "You're not autorized!");
		} else {
