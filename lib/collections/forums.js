...
Meteor.methods({
	addForum: function (name, icon) {
		if (!Roles.userIsInRole(this.userId, forumConfig.roles.addForumRoles)) {
	...
	editForum: function (id,name, icon) {
		if (!Roles.userIsInRole(this.userId, forumConfig.roles.editForumRoles)) {
			throw new Meteor.Error(403, "You're not autorized!");
		} else {
			Forums.update({
				_id: id
	...
	deleteForum: function (id) {
		if (!Roles.userIsInRole(this.userId, forumConfig.roles.deleteForumRoles)) {
			throw new Meteor.Error(403, "You're not autorized!");
		} else {
